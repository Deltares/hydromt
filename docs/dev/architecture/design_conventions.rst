...
- Input data is defined in a :ref:`data catalog <data_yaml>` and parsed by HydroMT to its associated Python type through the ``DataAdapter`` class.
...
- Most model components include both ``read`` and ``write`` methods for handling model-specific formats.
  ...
  The ``results`` component does not implement a ``write`` method.

- All public model methods may only accept arguments of basic Python types: ``str``, ``int``, ``float``, ``bool``, ``None``, ``list``, or ``dict``.
