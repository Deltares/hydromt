...
Each data source, is added to a data catalog yaml or toml file with a user-defined name.
...
.. code-block:: toml
    ...
    data_type = "RasterDataset/GeoDataset/GeoDataFrame/DataFrame"
    driver = "raster/raster_tindex/netcdf/zarr/vector/vector_table/csv/xlsx/xls"
    filesystem = "local/gcs/s3" [my_dataset.driver_kwargs]
    key = "value"
    ...
    [my_dataset.unit_mult]
    new_variable_name = "value"

    [[my_dataset.zoom_levels]]
    zoom_level = "zoom_resolution"
    [my_dataset.rename]
    old_variable_name = "new_variable_name"
...
The config file has an *optional* global **meta** data section:

- **root** (optional): root folder for all the data sources in the configuration file.
  If not provided the folder of where the configuration file is located will be used as root.
...
- **path**: path to the data file.
  Relative paths are combined with the global ``root`` option of the configuration file (if available) or the directory of the configuration file itself.
