...
class CfXmlUtilsExtended():
    ...
    def _new_extra_extent(self, region: str, top: str, bottom: str, left: str, right: str):
    ...
    def _create_xml_root(self, ns, schema_url, schema_name, extra_attributes = {}):
        ...
        attributes = {'xmlns:xsi': ns['xmlns:xsi'],
    ...
    def _add_geojson_layergroup(self,layergroups: ET.Element, geojson_files: List, region: str):
        """
        ...
        for geojson in geojson_files:
            assert geojson['shapeType'] in ['point', 'line', 'polygon'], 'shapeType should be point, line or polygon'
    ...
    def _add_layergroup_to_toplayer(self, toplayergroup: ET.Element, geometries: List, region: str, codebase: str):
        """
        ...
        if toplayergroup is None:
            ...
            layergroups = toplayergroup.findall('{http://www.wldelft.nl/fews}layerGroup')
            existing_layergroups = [layergroup.attrib['id'] for layergroup in layergroups]

        if not region in existing_layergroups:
            if codebase.lower() == 'ribasim':
                toplayergroup= self._add_network_layergroup(layergroups=toplayergroup, shape_files=geometries, region=region)
            elif codebase.lower() == 'wflow':
                toplayergroup = self._add_geojson_layergroup(layergroups=toplayergroup, geojson_files=geometries, region=region)
            else:
    ...
    def create_fews_xml_root(self, schema: str, version: str=None) -> ET.ElementTree:
        ...
        """
        _ns = {'xmlns': 'http://www.wldelft.nl/fews', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance'}
        _url = 'http://fews.wldelft.nl/schemas/version1.0/'
        if version is None:
            return self._create_xml_root(ns=_ns, schema_url=_url, schema_name=schema)
        else:
            return self._create_xml_root(ns=_ns, schema_url=_url, schema_name=schema, extra_attributes={'version':version})
    ...
    def append_group_to_topology_xml_object(self, xml_root: ET.Element, groupId: str, groupName: str):
        ...
        try:
            # find scheme
            nodes_tree = [n for n in xml_root.findall('{http://www.wldelft.nl/fews}nodes') \
                        if n.attrib['id'] == f'{groupId}'][0]
        except:
            # add scheme
            nodes_tree = ET.SubElement(xml_root, 'nodes', {'id': groupId, 'name': groupName})
        return xml_root
    ...
    def append_group_to_topologygroup_xml_object(self, xml_root: ET.ElementTree, parentgroupId: str,
        ...
        try:
            nodes_tree = [n for n in xml_root.findall('{http://www.wldelft.nl/fews}nodes') \
                        if n.attrib['id'] == f'{parentgroupId}'][0]
        except Exception as e:
            msg = f'Searching parent group {parentgroupId} raised Exception {e}'
            self.logger.error(msg)
            raise msg
        grps = [g for g in nodes_tree.findall('{http://www.wldelft.nl/fews}groupId') if g.text== childgroupId]
        if len(grps) < 1:
            self._add_child_with_text(nodes_tree, 'groupId', childgroupId)
        return xml_root


    def insert_extra_extent_into_explorer_xml_object(self, xml_root: ET.ElementTree,
    ...
    def add_geometries_to_defaults_spatial_display_xml_object(self,
                        xml_root: ET.ElementTree, geometries: List, region: str, codebase: str):
        """
        ...
        def _find_layergroup(xml_root: ET.ElementTree):
            ...
            for default in xml_root.iter('{http://www.wldelft.nl/fews}defaults'):
                geomap = default.find('{http://www.wldelft.nl/fews}geoMap')
                if len(geomap) > 0:
                    toplayergroups = geomap.findall('{http://www.wldelft.nl/fews}layerGroup')
                    if len(toplayergroups) > 0:
                        for toplayergroup in toplayergroups:
                            if toplayergroup.attrib['id'].lower() == codebase:
                                return toplayergroup
                    return ET.SubElement(geomap, 'layerGroup', {'id': codebase, 'name': codebase.title()})
                self.logger.warning('No GeoMap available, cannot add layergroup')
            return None

        toplayergroup = _find_layergroup(xml_root)
        toplayergroup = self._add_layergroup_to_toplayer(toplayergroup=toplayergroup,
                                                         geometries=geometries, region=region, codebase=codebase)

        return xml_root

    def add_geometries_to_explorer_xml_object(self, xml_root: ET.ElementTree, geometries: List, region: str, codebase: str):
        """
        Function to add network child layer group to a parent layer group
