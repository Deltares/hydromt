...
@main.command(short_help="Get and download examples.")
@click.option(
    "-m",
    "--model",
    type=str,
    help="Specify a model plugin to download its examples.",
    default=None,
)
@click.option(
    "-o",
    "--examples-out",
    type=click.Path(resolve_path=True, dir_okay=True, file_okay=False),
    help="Output folder of the examples.",
    default=None,
)
@quiet_opt
@verbose_opt
@click.pass_context
def examples(ctx, model, examples_out, quiet, verbose):
    """Download examples from hydromt core or plugins based on the installed version.

    Example usage for hydromt core to save in the default examples folder in current working directory
    hydromt get_examples

    Example usage for hydromt core to save in examples/hydromt folder
    hydromt get_examples -o examples/hydromt

    Example usage for hydromt wflow to save in examples/hydromt_wflow folder
    hydromt get_examples -m wflow -o examples/hydromt_wflow

    """  # noqa: E501
    if examples_out is None:
        examples_out = "examples"
    log_level = max(10, 30 - 10 * (verbose - quiet))
    logger = log.setuplog(
        "examples", join(examples_out, "hydromt-examples.log"), log_level=log_level
    )
    if model is None:
        model = "core"

    # TODO: for non-dev version, start publishing examples folder
    # as additionnal release data
    # easier to download than discover from github repo url
    if model == "core":
        version = "main" if __version__.endswith("dev0") else f"v{__version__}"
        examples_path = (
            "https://github.com/Deltares/hydromt/tree/{version}/examples".format(
                version=version
            )
        )
        examples_path_raw = "https://raw.githubusercontent.com/Deltares/hydromt/{version}/examples".format(
            version=version
        )
    else:
        if model in MODELS.plugins:
            version = MODELS.eps[model].distro.version
            version = "main" if version.endswith("dev0") else f"v{version}"
            # TODO: see if in the future the examples folder
            # can be retrieved from the repo itself
            # maybe a new Model attribute or something
            examples_path = "https://github.com/Deltares/hydromt_{model}/tree/{version}/examples".format(
                model=model, version=version
            )
            examples_path_raw = "https://raw.githubusercontent.com/Deltares/hydromt_{model}/{version}/examples".format(
                model=model, version=version
            )
        else:
            raise ValueError(
                f"HydroMT plugin {model} not found. Use of of {MODELS.keys()}"
            )

    try:
        logger.info(
            f"Getting and downloading examples for hydromt {model} version {version}."
        )
        logger.info(f"Download url: {examples_path}")
        logger.info(f"Output folder: {examples_out}")
        cli_utils.download_examples(
            examples_path=examples_path,
            examples_out=examples_out,
            examples_path_raw=examples_path_raw,
            logger=logger,
        )
