...
def test_cli_build_missing_arg_workflow(tmpdir):
    ...
    assert "Validation of step 1 (config.update) failed because of the following error:" in r.output
...
def test_cli_build_v0x_workflow(tmpdir):
    ...
    assert r.exit_code == 1
    assert "does not contain a `steps` section. Perhaps you're using a v0.x format?" in r.output
