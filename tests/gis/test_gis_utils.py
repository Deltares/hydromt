...
from hydromt.gis import gis_utils, raster_utils, vector_utils
from hydromt.gis.raster import RasterDataArray


def test_crs():
    bbox = [3, 51.5, 4, 52]  # NL
    assert gis_utils.utm_crs(bbox).to_epsg() == 32631
    assert gis_utils.parse_crs("utm", bbox).to_epsg() == 32631
    bbox1 = [-77.5, -12.2, -77.0, -12.0]
    assert gis_utils.utm_crs(bbox1).to_epsg() == 32718
    _, _, xattrs, yattrs = gis_utils._axes_attrs(gis_utils.parse_crs(4326))
    assert xattrs["units"] == "degrees_east"
    assert yattrs["units"] == "degrees_north"
    _, _, xattrs, yattrs = gis_utils._axes_attrs(gis_utils.utm_crs(bbox1))
...
def test_affine_to_coords():
    ...
    shape = (180, 360)
    coords = raster_utils._affine_to_coords(transform, shape)
...
def test_meridian_offset():
    ...
    for x0 in [-180, 0, -360, 1]:
        ...
        da1 = raster_utils._meridian_offset(da)
        ...
        for bbox in [
            ...
            da2 = raster_utils._meridian_offset(da, bbox=bbox)
    ...
    with pytest.raises(ValueError, match="This method is only applicable to data"):
        raster_utils._meridian_offset(da.raster.clip_bbox([0, 0, 10, 10]))
...
def test_transform_rotation():
    ...
    assert xs.ndim == 2
    assert ys.ndim == 2
    da = raster_utils.full_from_transform(transform, shape, crs=4326)
...
def test_area_res():
    ...
    assert raster_utils.cellres(0) == (111319.458, 110574.2727)


def test_gdf(world):
    country = world.iloc[[0], :].to_crs(3857)
    assert np.all(vector_utils._filter_gdf(world, country) == 0)
    idx0 = vector_utils._filter_gdf(world, bbox=[3, 51.5, 4, 52])[0]
    assert world.iloc[idx0,]["iso_a3"] == "NLD"
    with pytest.raises(ValueError, match="Unknown geometry mask type"):
        vector_utils._filter_gdf(world, geom=[3, 51.5, 4, 52])


def test_nearest(world, geodf):
    idx, _ = vector_utils.nearest(geodf, geodf)
    assert np.all(idx == geodf.index)
    idx, dst = vector_utils.nearest(geodf, world)
    assert np.all(dst == 0)
...
def test_spread():
    ...
    ds_out = raster_utils.spread2d(da_obs, da_friction=da_frc, da_mask=da_msk)
    ...
    with pytest.raises(ValueError, match='"nodata" must be a finite value'):
        raster_utils.spread2d(da_obs, nodata=np.nan)
...
class TestBBoxFromFileAndFilters:
    ...
    def test_gdf_bbox_crs_source_crs(
        ...
        bbox = gis_utils._bbox_from_file_and_filters(
    ...
    def test_does_not_filter(self, vector_data_with_crs: Path):
        bbox = gis_utils._bbox_from_file_and_filters(vector_data_with_crs)
        assert bbox is None
    ...
    def test_raises_valueerror(
        ...
        with pytest.raises(
            ...
            gis_utils._bbox_from_file_and_filters(
