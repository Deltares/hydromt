...
def test_crs():
    bbox = [3, 51.5, 4, 52]  # NL
    assert utils.utm_crs(bbox).to_epsg() == 32631
    assert utils.parse_crs("utm", bbox).to_epsg() == 32631
    bbox1 = [-77.5, -12.2, -77.0, -12.0]
    assert utils.utm_crs(bbox1).to_epsg() == 32718
    _, _, xattrs, yattrs = utils.axes_attrs(utils.parse_crs(4326))
    assert xattrs["units"] == "degrees_east"
    assert yattrs["units"] == "degrees_north"
    _, _, xattrs, yattrs = utils.axes_attrs(utils.utm_crs(bbox1))
...
def test_affine_to_coords():
    ...
    shape = (180, 360)
    coords = utils.affine_to_coords(transform, shape)
...
def test_meridian_offset():
    ...
    for x0 in [-180, 0, -360, 1]:
        ...
        for bbox in [
            ...
            da2 = utils.meridian_offset(da, bbox=bbox)
            assert (
    ...
    with pytest.raises(ValueError, match="This method is only applicable to data"):
        utils.meridian_offset(da.raster.clip_bbox([0, 0, 10, 10]))
...
def test_area_res():
    ...
    assert utils.cellres(0) == (111319.458, 110574.2727)


def test_gdf(world):
    country = world.iloc[[0], :].to_crs(3857)
    assert np.all(utils.filter_gdf(world, country) == 0)
    idx0 = utils.filter_gdf(world, bbox=[3, 51.5, 4, 52])[0]
    assert world.iloc[idx0,]["iso_a3"] == "NLD"
    with pytest.raises(ValueError, match="Unknown geometry mask type"):
        utils.filter_gdf(world, geom=[3, 51.5, 4, 52])


def test_nearest(world, geodf):
    idx, _ = utils.nearest(geodf, geodf)
    assert np.all(idx == geodf.index)
    idx, dst = utils.nearest(geodf, world)
    assert np.all(dst == 0)
    assert np.all(world.loc[idx, "name"].values == geodf["country"].values)
    gdf0 = geodf.copy()
    gdf0["iso_a3"] = ""
    gdf1 = utils.nearest_merge(geodf, world.drop(idx), max_dist=1e6)
...
def test_spread():
    ...
    ds_out = utils.spread2d(da_obs, da_friction=da_frc, da_mask=da_msk)
    ...
    with pytest.raises(ValueError, match='"nodata" must be a finite value'):
        utils.spread2d(da_obs, nodata=np.nan)
...
def test_create_vrt(tmpdir, rioda_large):
    ...
    utils.create_vrt(vrt_fn, files_path=files_path)
    ...
    with pytest.raises(ValueError, match="Either 'files' or 'files_path' is required"):
        utils.create_vrt(vrt_fn)
    with pytest.raises(IOError, match="No files found at "):
        utils.create_vrt(vrt_fn, files_path=os.path.join(path, "dummy_xyz", "*.abc"))
...
class TestBBoxFromFileAndFilters:
    ...
    def test_gdf_bbox_crs_source_crs(
        ...
        bbox = utils.bbox_from_file_and_filters(
    ...
    def test_shapely_input(self, shapely_bbox: Polygon, vector_data_with_crs: Path):
        bbox = utils.bbox_from_file_and_filters(vector_data_with_crs, bbox=shapely_bbox)
        assert all(map(lambda x: abs(x) > 180, bbox))

    def test_does_not_filter(self, vector_data_with_crs: Path):
        bbox = utils.bbox_from_file_and_filters(vector_data_with_crs)
        assert bbox is None
    ...
    def test_raises_valueerror(
        ...
        with pytest.raises(
            ...
            utils.bbox_from_file_and_filters(
