...
def test_region_from_model(tmpdir, world, mocker: MockerFixture):
    ...
    plugins = mocker.patch("hydromt.model.region.PLUGINS")
...
def test_parse_region_mesh_path(mocker: MockerFixture):
    ...
    xu_open_dataset_mock = mocker.patch(
        "hydromt.model.region.xu.open_dataset", return_value=ugrid_mock
    )
    mocker.patch("hydromt.model.region.isfile", return_value=True)
